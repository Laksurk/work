...
**【定义 2】**设 $H\le G$，对于 $a\in G$，称 $aH$ 为 $H$ 由 $a$ 生成的左**陪集（coset）**，$Ha$ 为 $H$ 由 $a$ 生成的右陪集. 
...
> **证明**　第二个等价即推论 5，下证第一个等价. 依次应用混合结合律、推论 2、定理 3：
> $$
> \begin{align}
...
**【定理 6】拉格朗日定理（Lagrange‘s Theorem）**：设$G$ 为有限群，$H\le G$，则 $\dfrac{|G|}{|H|}\in\Z$. 

> **证明**　群 $G$ 中 $H$ 的所有左陪集构成 $G$ 的划分（定理 5），且每个陪集的大小均为 $|H|$，故有 $|H|$ 整除 $|G|$. 

...
**【定义 1】**定义集合 $X_n=\N^+\cup[1,n]=\{1,2,3,\cdots,n\}$ $(n\ge 1)$.

**【定义 2】**对于集合 $X$，定义集合 $\cal P$$(X)=\{A:A\subseteq X\}$，即 $X$ 的所有子集组成的集合. 

**【定理 1】**对任意正整数 $n$，$(\cal P$$(X_n),\subseteq)$ 为偏序集. 

> **证明&emsp;**容易验证有 $A\subseteq A$（自反性），$A\ne B\land A\subseteq B\Rightarrow B\nsubseteq A$（反对称性），$A\subseteq B\land B\subseteq C\Rightarrow A\subseteq C$（传递性）. 

**【定理 2】**对于偏序集 $(\cal P$$(X_n),\subseteq)$，其上的莫比乌斯函数为
$$
\mu(A,B)=\left\{\begin{array}{l}
(-1)^{|B|-|A|} & A\subseteq  B\\
0 &\mathrm{otherwise}
\end{array}\right.
$$

> 证明略. 实际上，该定理极类似于 $(\N^+,\preccurlyeq)$ 上只保留所有 Square Free Number 的特殊情况. 可用补集运算 $A\backslash B$ 类比除法运算 $a/b$，各种相关结论的证明全部照搬即可. 

在 $(\cal P$$(X_n),\subseteq)$ 上应用（广义）莫比乌斯反演公式，得到：

**【定理 3】子集反演公式**：对于函数 $F,G:\cal P$$(X_n)\mapsto\R$，有
$$
G(A)=\sum\limits_{B:B\subseteq A}F(B)\ \ \Leftrightarrow\ \  F(A)=\sum\limits_{B:B\subseteq A}(-1)^{|A|-|B|}G(B)
$$